  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++)
  {
    ...
    for (k = block8x8 * 4; k < block8x8 * 4 + 4; k ++)
    {
      ...
      copy_image_data_4x4(&currImg[j_pos], &currSlice->mb_rec[curr_plane][joff], i_pos, ioff);
        ...
        byte predmode = p_Vid->ipredmode[i4][j4];
        pm4x4[m++] = predmode;
        
    }
      //int EHB4x4[5]={0};
      calculate8x8(pm4x4, EHB);
...
void calculate8x8(int pm4x4[4], int EHB[5]){
    int n = 4;
    int mode8x8 = 0;
    for (int i=0; i<4; i++) {
        int mode = pm4x4[i];
    ...
    if (n == 0) {
      ...
      mode8x8 /= n;
      if (mode8x8 <=26 || mode8x8 >= 154) {
        makeEHB(1, EHB);
      } else if (mode8x8 >=64 && mode8x8 <= 116){
        makeEHB(2, EHB);
      }else if (mode8x8 >26 && mode8x8 < 64){
        makeEHB(3, EHB);
      }else if (mode8x8 > 116 && mode8x8 < 154){
        makeEHB(4, EHB);
      }
    }
}

void makeEHB(int region, int EHB[5]){
