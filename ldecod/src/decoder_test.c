...
//    int numberOfEHBArr[100];
  ...
  do
  {
      int frameNo = 0;
      //int numberOfEHB = 0;
      iRet = decodeOneFrame(&pDecPicList, frameEHB);
      //numberOfEHBArr[frameNo] = numberOfEHB;
      numberOfFrame++;
  ...
  printf("%d frames are decoded.\n", iFramesDecoded);
    ...
    for (int i=0; i<numberOfFrame; i++) {
        int EHBK = 0;
        for (int j=0; j<5 && i!=numberOfFrame-1; j++) {
            EHBK += abs(frameEHB[i+1][j] - frameEHB[i][j]);
        }
//        if (i==0) {
//            printf("EHBD(%d)=%d, ", 2, EHBK);
//        }
//        else{
//            printf("EHBD(%d)=%d, ", i+1, EHBK);
//        }
        
        if (EHBK >= 25) {
            printf("Scene change occurs at frame %d\n", i+1);
        }
    }
