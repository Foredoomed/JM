...
int decode_one_frame(DecoderParams *pDecoder)
{
  ...
  
  {
      //int frameEHB[10000];
      //int n =0;
      memset(pDecoder->frameEHB, 0, sizeof(pDecoder->frameEHB));
    for(iSliceNo=0; iSliceNo<p_Vid->iSliceNumOfCurrPic; iSliceNo++)
    {
      ...
      p_Vid->erc_mvperMB += currSlice->erc_mvperMB;
        ...
        //sysc5404
        for (int i=0; i<5; i++) {
            pDecoder->frameEHB[i] += currSlice->EHB[i];
        }
        
        //n += currSlice->numberOfEHB;
    }
      
      //sysc5404
      //pDecoder->numberOfEHB = n;
  }
#if MVC_EXTENSION_ENABLE
...
void decode_one_slice(Slice *currSlice)
{
  ...
  //reset_ec_flags(p_Vid);
    //currSlice->numberOfEHB = n;
    memset(currSlice->EHB, 0, sizeof(currSlice->EHB));
    for (int i =0; i<5; i++) {
        currSlice->EHB[i] = EHB[i];
    }
